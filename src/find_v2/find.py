...
def is_probably_apn_v2(possibly_apn, sample=None):
...
def find_apn_sample_in_excel_table_v2(df, test_page_number):
        ...
        for index, row in df.head(row_search_tolerance).iterrows():
            for column, cell_value in row.items():
                ...
                for possible_apn in possible_apns:
                    if is_probably_apn_v2(possible_apn):
                        sample = possible_apn
                        break
                if sample:
                    break
...
def firstApnRowOfLastTable_v2(df):
    last_row = df.iloc[-1]
    # first_nested_row = last_row[columns[2]][0]
    # print("firstApnRowOfLastTable")
    # print(first_nested_row)
    apn = apn_value_of_first_nested_row_v2(last_row)
    # print("page_number")
    # print(str(page_number))
    return apn

def apn_value_of_first_nested_row_v2(row):
...
def are_values_similar_v2(str1, str2):
    str1 = str(str1)
    str2 = str(str2)
...
def sort_excel_files_by_page_number_v2(lst):
...
# Extract APNs from excel spreadsheet files. target_page argument is for extracting from only one page. If -1, then extract all pages.
def find_tables_and_parcels_v2(city_output_directory, target_page = -1):
    ...
    excel_doc_filepaths = sort_excel_files_by_page_number_v2(excel_doc_filepaths)
    ...
    for excel_doc_filepath in excel_doc_filepaths:
        ...
        if target_page > -1 and page_number != target_page:
            continue
        ...
        if len(new_df) > 0:
            ...
            if is_table_next_to_previous_apn_table:
                apn_sample = apn_value_of_first_nested_row_v2(last_row)

        if apn_sample == None:
            apn_sample = find_apn_sample_in_excel_table_v2(df, page_number)
        ...
        for index, row in df.iterrows():
            for column, cell_value in row.items():
                ...
                for possible_apn in pull_possible_apns_from_cell_value(cell_value):
                    ...
                    # print("sample: ")
                    # print(apn_sample)
                    if apn_sample and is_probably_apn_v2(possible_apn, apn_sample):
                        
                        

                        new_apn = str(possible_apn)
                        ...
                        if isinstance(new_apn, str) and not new_apn.lower().strip() == "nan" and not new_apn.isspace() and len(new_apn) > 0:
                            ...
                            if apn_sample == None or are_values_similar_v2(new_apn, apn_sample) == True:
